...
parfor day=1:31
    ...
    tic
    t=0;
    genes{1}=GenerateParental2023(Population,EncodingLength,day,toOrder);
    %top_gene=genes{1}{1};
    ...
    for k=2:Generation
        ...
        genes{k}=Choose2023(Parent,Child1,Child2);
        t=t+toc;
        %Goal(k)=top_gene(1,EncodingLength);
        disp(t);
    end
    %Fig=plot(1:Generation,Goal,'b.-');
    %title(strcat("2023年1月",num2str(day),"日智能算法迭代结果"));
    %Name=strcat("2023-1-",num2str(day),".jpg");
    %saveas(Fig, Name);
    disp(day);
    Genes{day}=genes;
end
%str=strcat("Genes",num2str(int(rand*100)),".mat");
save("GenesOUT9.mat","Genes");
delete(P);
...
function Genes=GenerateParental2023(Population,EncodingLength,day,toOrder)
    ...
    for p=1:Population
        ...
        [~,value,overflow]=SAforOrderIN9(Sequence,day,toOrder);
...
function superior=Choose2023(A,B,C)
    %遗传算法选择优秀基因型作为新的亲本
    %A,B,C分别为待选择的基因型
    Merge=[A;B;C];
