%clc;clear;close all;
function [order,result,overflow]=SAforOrder(index,day)
...
num_node=81;
    %%init loading--E3
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    edge=Status{3}(index,1);
    start=Status{3}(index,2);
    term=Status{3}(index,3);
    contain=Status{3}(index,4);
    predict=Status{3}(index,5+day-1);
    predict=min(predict,contain);
    %rate=sum(predict);%正在跑的总流量
    Total=sum(Status{1}(index,4));%流向DC5的需要分配的流量
    ...
    for i=1:num_edge%考虑E3的运输能力
        if (predict(i)+order(i)>contain(i))
            ...
            if (reOrder<=(contain(i)-predict(i))/2)
                order(i)=order(i)+reOrder;
                reOrder=0;
            else
                order(i)=order(i)+(contain(i)-predict(i))/2;
                reOrder=reOrder-(contain(i)-predict(i))/2;
            end
        end
    end
    flag=1;%还存在有容量的边;
    ...
    for i=1:num_edge
        ...
        node=start(edge(i));%取出N1中的点
        runningPop(node)=runningPop(node)+predict(i)+order(i);
        node=term(edge(i));%取出N3中的点
        runningPush(node)=runningPush(node)+predict(i)+order(i);
        clear node
    end
        for i=1:num_edge%处理点的冲突
    ...
    changed=Effected(totalPredict,order,index);
    Res=CalcGoal(changed,totalContain,totalLen);
    for Layer=1:MarkovLength
        upFlow=Generate(predict,contain,start,term,order);
        while (CheckValid(order+upFlow)==0)
            upFlow=Generate(predict,contain,start,term,order);
        end
        changed=Effected(totalPredict,order+upFlow,index);
...
function upFlow=Generate(predict,contain,res)
    ...
    while(shake>0)
        ...
        take=shake*0.2;
        if (res(ind_sort(p))+upFlow(ind_sort(p))-take>0)%能够被调节
