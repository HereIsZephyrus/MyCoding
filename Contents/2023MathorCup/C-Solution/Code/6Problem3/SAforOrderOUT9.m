%clc;clear;close all;
function [order,result,overFlow]=SAforOrderOUT9(index,day,toOrder)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    local=find(index==1);
    edge=Status{4}(local,1);
    start=Status{4}(local,2);
    term=Status{4}(local,3);
    contain=Status{4}(local,4);
    predict=Status{4}(local,5+day-1);
    predict=min(predict,contain);
    predict=Clean(predict);
    %rate=sum(predict);%正在跑的总流量
    Total=toOrder;%流向DC9的需要分配的流量
    num_edge=length(edge);%E4集合的大小
    ...
    Rate=0.9;
    MarkovLength=500;
    totalContain=Status{4}(:,4);%全部边为了得到目标量
    totalPredict=Status{4}(:,5+day-1);
...
function upFlow=Generate(predict,contain,res)
    ...
    p=1;
    while(shake>0)
        if (shake<1 && predict(ind_sort(p))+res(ind_sort(p))+upFlow(ind_sort(p))>shake)%消除抖动循环
            upFlow(ind_sort(p))=upFlow(ind_sort(p))-shake;
            shake=0;
            break
        end
        take=shake*0.2;
        while (predict(ind_sort(p))+res(ind_sort(p))+upFlow(ind_sort(p))-take<=0)%不能够被调节
            take=take*0.8;
            if (take<1)
                overFlow=1;
                return
    ...
    while(shake<0)
        if (shake>-1 && predict(ind_sort(p))+res(ind_sort(p))+upFlow(ind_sort(p))-shake<=contain(ind_sort(p)))%消除抖动循环
            upFlow(ind_sort(p))=upFlow(ind_sort(p))-shake;%减负等于加正下同理;
            shake=0;
            break
        end
        take=shake*0.2;
        if (predict(ind_sort(p))+res(ind_sort(p))+upFlow(ind_sort(p))+take<=contain(ind_sort(p)))%能够被调节
            upFlow(ind_sort(p))=upFlow(ind_sort(p))-take;
            shake=shake-take;
        end
        p=mod(p,n)+1;
