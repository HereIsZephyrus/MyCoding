%clc;clear;close all;
function [order,result,overFlow]=SAforOrderIN9(index,day,toOrder)
    load StatusAmong9total.mat
    load maxVolume.mat
    reOrder=0;
    rng('shuffle');
    num_node=81;
    ...
    %循环解决冲突
    while (round(reOrder*100)>0 && (nnz(shutPop)+nnz(shutPush))>0)%还有未分配的流量并且不能所有节点都关闭了
        for i=1:num_edge%考虑E3的运输能力
            if (shutPop(start(i))==0 || shutPush(term(i))==0)
                continue
            end
        ...
        flag=1;%还存在有容量的边;
        while (round(reOrder*100)>0 && flag==1)
            flag=0;
            for i=1:num_edge%考虑E3的运输能力
                if (shutPop(start(i))==0 || shutPush(term(i))==0)
                    continue
