...
fprintf('\n待测圆柱的质量是%4.2fg,待测圆柱的直径是%5.2fcm,其中两根圆柱在测量上完全一致故不做区分\n',columnm,column_D);
...
[k,b]=LeastSquares_Line(Average,Distance2);
fplot(@(x) k*x+b,'b');
grid;
xlabel('转动惯量 \it{J}');
ylabel('据质心距离 \it{D}^{2}');
legend('原始数据曲线','拟合直线','Location','northwest');
title('验证平行轴定理——J-D^{2}曲线与最小二乘拟合');
hold off;
fprintf('拟合直线函数表达式为y=%.4fx%.4f\n',k,b);
