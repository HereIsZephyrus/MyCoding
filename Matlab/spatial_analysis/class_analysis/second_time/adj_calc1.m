...
for i=1 : times
    ...
    writematrix(part_adj,'adj_class1.xlsx',Sheet=i);
    writematrix(part_count,'count_class1.xlsx',Sheet=i);
end
clear i part_adj part_count
writematrix(adj,'adj_class1.xlsx',Sheet=8);
writematrix(count./roll_in,'count_class1.xlsx',Sheet=8);
...
clear i
writematrix(var_vec./roll_in,'vec_class1.csv');
writematrix(mean_vec./roll_in,'mean_class1.csv');
save trace_class1.mat vec adj count
...
function [x,y] = calc_vec(index)
    ...
    num = 75;
    x = X(1:num,index+1) - X(1:num,index);
    y = Y(1:num,index+1) - Y(1:num,index);
end