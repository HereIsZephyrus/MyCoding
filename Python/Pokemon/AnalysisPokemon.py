...
for item in areas:
    ...
    for data in raw_pekemons:
        ...
        #print(attribute)
        #c=input()
        point=data.find('pokemon.data_egg_group[p.egg_group_b')
        ...
        for i in range(1,6):
            ...
            point=point+10
        power.append(temp)
        #print(f'{len(power)}+{len(egg)}')
        #c=input()
    file.close()
    f_output=open(f'./base/{item}.csv','w', newline='',encoding="utf-8") 
